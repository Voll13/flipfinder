...
CREATE TABLE IF NOT EXISTS rental_profiles (id INTEGER PRIMARY KEY AUTOINCREMENT, profile_key TEXT NOT NULL UNIQUE, marketplace TEXT NOT NULL, vertical TEXT NOT NULL DEFAULT 'rental' CHECK(vertical='rental'), country_code TEXT NOT NULL, source_city TEXT NOT NULL, display_name_ru TEXT NOT NULL, search_url TEXT NOT NULL, destination_key TEXT NOT NULL, enabled INTEGER NOT NULL DEFAULT 1, bootstrap_completed_at TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
CREATE INDEX IF NOT EXISTS idx_rental_profiles_enabled ON rental_profiles(enabled, marketplace, source_city);
CREATE TABLE IF NOT EXISTS rental_listings (id INTEGER PRIMARY KEY AUTOINCREMENT, source TEXT NOT NULL, source_listing_id TEXT NOT NULL, canonical_url TEXT NOT NULL, title TEXT NOT NULL, country_code TEXT NOT NULL, city TEXT NOT NULL, published_at TEXT, district TEXT, address_text TEXT, latitude TEXT, longitude TEXT, property_type TEXT, rooms TEXT, area_m2 TEXT, floor_label TEXT, total_floors INTEGER, rent_price_pln INTEGER NOT NULL, admin_fee_pln INTEGER, utilities_text TEXT, deposit_pln INTEGER, furnished INTEGER, balcony INTEGER, elevator INTEGER, parking INTEGER, pets_policy TEXT CHECK(pets_policy IN ('allowed','not_allowed','negotiable') OR pets_policy IS NULL), advertiser_type TEXT CHECK(advertiser_type IN ('private','agency') OR advertiser_type IS NULL), agency_name TEXT, description TEXT, image_urls TEXT NOT NULL DEFAULT '[]', main_image_url TEXT, source_attributes_json TEXT NOT NULL DEFAULT '{}', first_seen_at TEXT NOT NULL, last_seen_at TEXT NOT NULL, is_active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, UNIQUE(source, source_listing_id));
CREATE INDEX IF NOT EXISTS idx_rental_listings_city_active_seen ON rental_listings(city, is_active, last_seen_at DESC);
CREATE INDEX IF NOT EXISTS idx_rental_listings_source_id ON rental_listings(source, source_listing_id);
CREATE TABLE IF NOT EXISTS rental_price_history (id INTEGER PRIMARY KEY AUTOINCREMENT, rental_listing_id INTEGER NOT NULL, rent_price_pln INTEGER NOT NULL, observed_at TEXT NOT NULL, FOREIGN KEY(rental_listing_id) REFERENCES rental_listings(id) ON DELETE CASCADE);
CREATE INDEX IF NOT EXISTS idx_rental_price_history_listing ON rental_price_history(rental_listing_id, observed_at);
CREATE TABLE IF NOT EXISTS rental_publications (id INTEGER PRIMARY KEY AUTOINCREMENT, rental_listing_id INTEGER NOT NULL, destination_key TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','sent','failed','uncertain')), telegram_message_id INTEGER, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, sent_at TEXT, last_error TEXT, UNIQUE(rental_listing_id, destination_key), FOREIGN KEY(rental_listing_id) REFERENCES rental_listings(id) ON DELETE CASCADE);
CREATE INDEX IF NOT EXISTS idx_rental_publications_status ON rental_publications(status, updated_at);
"""
