...
class OLXScraper(BaseScraper):
    ...
    def _extract_prerendered_state(self, html: str) -> dict[str, Any]:
        ...
        # Chrome's "View Source" save represents source lines as table cells,
        # so the real script tag is no longer a DOM element.  Its decoded text
        # still contains the identical embedded state assignment.
        source = script.get_text() if script else soup.get_text()
        match = _STATE_RE.search(source)
        if not match: raise ScraperError("OLX __PRERENDERED_STATE__ assignment is missing")
        try: state = json.loads(json.loads(match.group(1)))
        except (json.JSONDecodeError, TypeError) as exc: raise ScraperError("OLX __PRERENDERED_STATE__ is not valid JSON") from exc
        if not isinstance(state, dict): raise ScraperError("OLX __PRERENDERED_STATE__ must decode to an object")
        return state
